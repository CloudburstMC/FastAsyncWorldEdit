...
import com.boydti.fawe.cloudburst.adapter.DelegateLock;
import com.boydti.fawe.cloudburst.adapter.mc1152.nbt.LazyCompoundTag1152;
import com.boydti.fawe.cloudburst.adapter.mc1152.BukkitAdapter1152;
...
public class BukkitGetBlocks1152 extends CharGetBlocks {

    private static final Logger log = LoggerFactory.getLogger(BukkitGetBlocks1152.class);

    private static final Function<BlockPosition, BlockVector3> posNms2We = v -> BlockVector3.at(v.getX(), v.getY(), v.getZ());
    private static final Function<TileEntity, CompoundTag> nmsTile2We = tileEntity -> new LazyCompoundTag1152(Suppliers.memoize(() -> tileEntity.save(new NBTTagCompound())));
    ...
    public BukkitGetBlocks1152(World world, int chunkX, int chunkZ) {
        this(((CraftWorld) world).getHandle(), chunkX, chunkZ);
    }

    public BukkitGetBlocks1152(WorldServer world, int chunkX, int chunkZ) {
        this.world = world;
        this.chunkX = chunkX;
        this.chunkZ = chunkZ;
    }
    ...
    public BiomeType getBiomeType(int x, int y, int z) {
        ...
        return base != null ? CloudburstAdapter.adapt(CraftBlock.biomeBaseToBiome(base)) : null;
    }
    ...
    public CompoundTag getTile(int x, int y, int z) {
        ...
        return new LazyCompoundTag1152(Suppliers.memoize(() -> tileEntity.save(new NBTTagCompound())));
    }
    ...
    public CompoundTag getEntity(UUID uuid) {
        ...
        if (entity != null) {
            ...
            return CloudburstAdapter.adapt(bukkitEnt).getState().getNbtData();
        }
    ...
    private void updateGet(BukkitGetBlocks1152 get, Chunk nmsChunk, ChunkSection[] sections, ChunkSection section, char[] arr, int layer) {
        synchronized (get) {
    ...
    public Chunk ensureLoaded(net.minecraft.server.v1_15_R1.World nmsWorld, int chunkX, int chunkZ) {
        return BukkitAdapter1152.ensureLoaded(nmsWorld, chunkX, chunkZ);
    }
            ...
            synchronized (nmsChunk) {
                ...
                for (int layer = 0; layer < 16; layer++) {
                    ...
                    if (existingSection == null) {
                        newSection = BukkitAdapter1152.newChunkSection(layer, setArr, fastmode);
                        if (BukkitAdapter1152.setSectionAtomic(sections, null, newSection, layer)) {
                    ...
                    BukkitAdapter1152.fieldTickingBlockCount.set(existingSection, (short) 0);

                    //ensure that the server doesn't try to tick the chunksection while we're editing it.
                    DelegateLock lock = BukkitAdapter1152.applyLock(existingSection);
                    ...
                    synchronized (this) {
                        synchronized (lock) {
                            ...
                            newSection = BukkitAdapter1152.newChunkSection(layer, this::load, setArr, fastmode);
                            if (!BukkitAdapter1152.setSectionAtomic(sections, existingSection, newSection, layer)) {
                ...
                if (biomes != null) {
                    ...
                    for (int z = 0, i = 0; z < 16; z++) {
                        for (int x = 0; x < 16; x++, i++) {
                            ...
                            if (biome != null) {
                                final Biome craftBiome = CloudburstAdapter.adapt(biome);
                    ...
                    syncTasks[1] = () -> {
                        for (final CompoundTag nativeTag : entities) {
                            ...
                            if (idTag == null || posTag == null || rotTag == null) {
                                getLogger(BukkitGetBlocks1152.class).debug("Unknown entity tag: " + nativeTag);
                                continue;
                            }
                ...
                if (bitMask == 0 && biomes == null && !lightUpdate) {
                    ...
                    callback = () -> {
                        ...
                        nmsChunk.markDirty();
                        // send to player
                        BukkitAdapter1152.sendChunk(nmsWorld, chunkX, chunkZ, finalMask, finalLightUpdate);
                        if (finalizer != null) {
                            finalizer.run();
                        }
                    };
    ...
    public synchronized char[] update(int layer, char[] data) {
        ...
        DelegateLock lock = BukkitAdapter1152.applyLock(section);
        synchronized (lock) {
            lock.untilFree();
            lock.setModified(false);
            // Efficiently convert ChunkSection to raw data
            try {
                FAWESpigotV115R2 adapter = ((FAWESpigotV115R2) WorldEditPlugin.getInstance().getAdapter());

                final DataPaletteBlock<IBlockData> blocks = section.getBlocks();
                final DataBits bits = (DataBits) BukkitAdapter1152.fieldBits.get(blocks);
                final DataPalette<IBlockData> palette = (DataPalette<IBlockData>) BukkitAdapter1152.fieldPalette.get(blocks);
        ...
        if (aggressive) {
            ...
            for (int i = 0; i < 16; i++) {
                ...
                try {
                    ...
                    final DataPalette<IBlockData> palette = (DataPalette<IBlockData>) BukkitAdapter1152.fieldPalette.get(blocksExisting);
                    int paletteSize;
