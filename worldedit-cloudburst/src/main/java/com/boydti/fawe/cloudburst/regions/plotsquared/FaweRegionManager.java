package com.boydti.fawe.cloudburst.regions.plotsquared;
...
public class FaweRegionManager extends RegionManager {
    ...
    public boolean setCuboids(final PlotArea area, final Set<CuboidRegion> regions, final Pattern blocks, final int minY, final int maxY) {
        ...
        TaskManager.IMP.async(() -> {
            synchronized (FaweRegionManager.class) {
                World world = CloudburstAdapter.adapt(getWorld(area.getWorldName()));
    ...
    public boolean handleClear(final Plot plot, final Runnable whenDone, final PlotManager manager) {
        ...
        TaskManager.IMP.async(() -> {
            synchronized (FaweRegionManager.class) {
                ...
                EditSession editSession = new EditSessionBuilder(CloudburstAdapter.adapt(getWorld(hybridPlotWorld.getWorldName()))).checkMemory(false).fastmode(true).limitUnlimited().changeSetNull().autoQueue(false).build();
                ...
                if (!hybridPlotWorld.PLOT_SCHEMATIC || !Settings.Schematics.PASTE_ON_TOP) {
                    ...
                    for (int x = pos1.getX(); x <= pos2.getX(); x++) {
                        for (int z = pos1.getZ(); z <= pos2.getZ(); z++) {
                            clipboard.setBiome(x, 0, z, biome); //todo 3D biomes?
                        }
                    }
    ...
    public void swap(final Location pos1, final Location pos2, final Location pos3, final Location pos4, final Runnable whenDone) {
        ...
        TaskManager.IMP.async(() -> {
            synchronized (FaweRegionManager.class) {
                ...
                World pos1World = CloudburstAdapter.adapt(getWorld(pos1.getWorld()));
                World pos3World = CloudburstAdapter.adapt(getWorld(pos3.getWorld()));
    ...
    public void setBiome(CuboidRegion region, int extendBiome, BiomeType biome, String world, Runnable whenDone) {
        ...
        TaskManager.IMP.async(() -> {
            synchronized (FaweRegionManager.class) {
                EditSession editSession = new EditSessionBuilder(CloudburstAdapter.adapt(getWorld(world))).checkMemory(false).fastmode(true).limitUnlimited().changeSetNull().autoQueue(false).build();
