    ...
    public CompletableFuture<Boolean> execute(final PlotPlayer<?> player, String[] args, RunnableVal3<Command, Runnable, Runnable> confirm, RunnableVal2<Command, CommandResult> whenDone) throws CommandException {
        ...
        confirm.run(this, () -> {
            ...
            TaskManager.IMP.async(() -> {
                EditSession session = new EditSessionBuilder(CloudburstAdapter.adapt(Bukkit.getWorld(plot.getArea().getWorldName())))
                        .autoQueue(false)
                        .checkMemory(false)
                        .allowedRegionsEverywhere()
                        .player(CloudburstAdapter.adapt(Bukkit.getPlayer(player.getUUID())))
                        .limitUnlimited()
                        .build();
