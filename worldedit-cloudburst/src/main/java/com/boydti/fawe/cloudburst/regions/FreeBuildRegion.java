...
import com.boydti.fawe.cloudburst.wrapper.AsyncWorld;
...
public class FreeBuildRegion extends BukkitMaskManager {
    ...
    public FaweMask getMask(Player player, MaskType type) {
        ...
        World bukkitWorld = CloudburstAdapter.adapt(player.getWorld());
        ...
        BlockBreakEvent event = new BlockBreakEvent(block, ((CloudburstPlayer) player).getPlayer());

        return new FaweMask(new CuboidRegion(pos1, pos2)) {

            @Override
            public boolean isValid(Player player, MaskType type) {
                return bukkitWorld == CloudburstAdapter.adapt(player.getWorld())
