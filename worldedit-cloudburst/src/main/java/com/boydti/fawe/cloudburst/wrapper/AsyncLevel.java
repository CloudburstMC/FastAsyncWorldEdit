...
import com.sk89q.worldedit.cloudburst.CloudburstWorld;
 ...
 */
public class AsyncLevel extends PassthroughExtent implements ChunkManager {
     ...
     */
    @Deprecated
    public AsyncLevel(Level parent, boolean autoQueue) {
        this(parent, FaweAPI.createQueue(new CloudburstWorld(parent), autoQueue));
    }

    public AsyncLevel(String level, boolean autoQueue) {
        this(Server.getInstance().getLevelByName(level), autoQueue);
    }
     ...
     */
    @Deprecated
    public AsyncLevel(Level parent, Extent extent) {
        super(extent);
        this.parent = parent;
     ...
     */
    public static AsyncLevel wrap(Level level) {
        if (level instanceof AsyncLevel) {
            return (AsyncLevel) level;
        }
        return new AsyncLevel(level, false);
    }
     ...
     */
    public static synchronized AsyncLevel create(final WorldCreator creator) {
