...
public class BukkitPlayerBlockBag extends BlockBag implements SlottableBlockBag {
    ...
    public void fetchBlock(BlockState blockState) throws BlockBagException {
        ...
        for (int slot = 0; slot < items.length; ++slot) {
            ...
            if (!CloudburstAdapter.equals(blockState.getBlockType(), bukkitItem.getType())) {
                // Type id doesn't fit
                continue;
            }
    ...
    public void storeBlock(BlockState blockState, int amount) throws BlockBagException {
        ...
        if (freeSlot > -1) {
            items[freeSlot] = CloudburstAdapter.adapt(new BaseItemStack(blockState.getBlockType().getItemType(), amount));
            return;
        }
    ...
    public BaseItem getItem(int slot) {
        loadInventory();
        return CloudburstAdapter.adapt(items[slot]);
    }
    ...
    public void setItem(int slot, BaseItem block) {
        ...
        items[slot] = CloudburstAdapter.adapt(stack);
    }
